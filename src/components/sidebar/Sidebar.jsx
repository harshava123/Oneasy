...
import { MdOutlineSubscriptions, MdOutlineArticle, MdMenu, MdClose } from 'react-icons/md';
...
function Sidebar() {
  ...
  return (
    <>
      ...
      {/* Desktop Sidebar */}
      <div className={`${isCollapsed ? 'w-[70px]' : 'w-[240px]'} h-screen bg-white fixed left-0 top-0 flex flex-col hidden lg:flex transition-all duration-300 border-r border-gray-200`}>
        {/* Logo Section with Toggle Button */}
        <div className={`${isCollapsed ? 'px-3' : 'px-6'} pt-4 pb-4 flex justify-between items-center transition-all duration-300 border-b border-gray-100`}>
          <img src={logo} alt="OnEasy Logo" className={`${isCollapsed ? 'h-8' : 'h-10'} w-auto transition-all duration-300 ${isCollapsed ? 'mx-auto' : ''}`} />
          {!isCollapsed && (
            <button
      ...
      {/* Navigation */}
      <nav className={`flex-1 ${isCollapsed ? 'px-2' : 'px-4'} pt-3 space-y-2.5 transition-all duration-300`}>
        {menuItems.map((item, index) => (
          <Link 
            to={item.path} 
            key={index} 
            className={`flex items-center ${isCollapsed ? 'justify-center px-2' : 'space-x-3 px-3'} py-2 rounded-lg transition-all duration-200 ${
          ...
          >
            <span className={`${isCollapsed ? 'w-auto' : 'w-5'} flex justify-center`}>{React.cloneElement(item.icon, {
      ...
      <div className="mt-auto border-t border-gray-100">
        <div className={`${isCollapsed ? 'p-2' : 'p-4'} transition-all duration-300`}>
          ...
          >
            <div className={`flex items-center ${isCollapsed ? 'justify-center' : 'space-x-3'} cursor-pointer p-2 hover:bg-gray-50 rounded-lg transition-all duration-200`}>
              <div className={`${isCollapsed ? 'w-8 h-8' : 'w-6 h-6'} bg-[#01334C] rounded-full flex items-center justify-center text-white text-xs transition-all duration-300`}>
                A
              </div>
              {!isCollapsed && (
                <>
            ...
            {isProfileOpen && (
              <div className={`absolute bottom-full ${isCollapsed ? 'left-full ml-2' : 'left-0'} mb-1 ${isCollapsed ? 'w-auto' : 'w-full'} bg-white rounded-lg shadow-lg py-2 z-50`}>
                <button 
                  onClick={() => {
      ...
      {isMobileMenuOpen && (
        ...
        {/* Debug info */}
        {console.log('Mobile sidebar rendering, isMobileMenuOpen:', isMobileMenuOpen)}
            ...
            <Link 
              ...
              className={`flex items-center space-x-3 px-3 py-2.5 rounded-lg transition-colors duration-200 ${
                (item.path === '/client' ? location.pathname === '/client' : 
                 item.text === 'Registrations' ? isRegistrationsActive : 
